    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {clients.length === 0 && (
              <tr><td colSpan={5} className="text-center p-8 text-gray-400">Aucun client</td></tr>
            )}
            {clients.map((c) => (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">{editClient ? 'Modifier' : 'Ajouter'} un client</h2>
            <div className="space-y-3">
              ...
              <input className="w-full border rounded-lg p-2" placeholder="Téléphone" value={form.telephone} onChange={e => setForm({...form, telephone: e.target.value})} />
              <textarea className="w-full border rounded-lg p-2" placeholder="Adresse" value={form.adresse} onChange={e => setForm({...form, adresse: e.target.value})} />
            </div>
