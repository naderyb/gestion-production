...
export default function Produits() {
  ...
  async function fetchProduits() {
    try {
      ...
      setProduits([]);
      setError("Erreur reseau lors du chargement des produits");
    }
  }
          ...
          <tbody>
            ...
            {produits.map((p) => (
              <tr key={p.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-4">
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${p.stock <= p.stock_minimum ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                  >
                    {p.stock}
                  </span>
                </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">
              {editProduit ? "Modifier" : "Ajouter"} un produit
            </h2>
            <div className="space-y-3">
              ...
              <input
                ...
                type="number"
                value={form.cout_production}
                onChange={(e) =>
                  setForm({ ...form, cout_production: e.target.value })
                }
              />
            </div>
