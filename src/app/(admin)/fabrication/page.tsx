  ...
  async function fetchOrdres() {
    try {
      ...
      if (!res.ok) {
        setOrdres([]);
        setError(
          data?.error || "Impossible de charger les ordres de fabrication",
        );
        return;
      }
      ...
      setOrdres([]);
      setError("Erreur reseau lors du chargement des ordres");
    }
  }
          ...
          <div key={o.id} className="bg-white rounded-xl shadow p-6">
            <div className="flex justify-between items-start">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${statutColors[o.statut]}`}
                  >
                    {statutLabels[o.statut]}
                  </span>
                </div>
                <p className="text-gray-500 text-sm">
                  Commande #{o.commande_ref} • Quantité :{" "}
                  <strong>{o.quantite}</strong>
                </p>
            ...
            {o.date_debut && (
              <div className="mt-3 pt-3 border-t flex gap-6 text-sm text-gray-500">
                <span>
                  Début : {new Date(o.date_debut).toLocaleDateString("fr-FR")}
                </span>
                {o.date_fin && (
                  <span>
                    Fin : {new Date(o.date_fin).toLocaleDateString("fr-FR")}
                  </span>
                )}
              </div>
            )}
